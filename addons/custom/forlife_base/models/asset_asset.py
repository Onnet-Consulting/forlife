...
class AssetsAssets(models.Model):
    ...
    asset_account = fields.Many2one('account.account', 'Asset Account', company_dependent=True)
    use_ful_month = fields.Integer('UseFul month')
    doc_date = fields.Date('Doc Date')
    original_cost = fields.Float('Original Cost')
    depr_debit_account = fields.Many2one('account.account', string='Depr Debit Account', company_dependent=True)
    depr_credit_account = fields.Many2one('account.account', string='Depr Credit Account', company_dependent=True)
