...
"""
    ...
    @api.model
    def get_pos_order_detail_information(self, pos_reference, brand_code):
        ...
        if brand_code == 'TKL':
            ...
            for line in order.lines.filtered(lambda f: not f.is_promotion and f.qty != 0):
                ...
                product_detail.update({
                    key: {
                        'product_name': (product_detail.get(key) or {}).get('product_name') or line.product_id.name,
                        'product_barcode': (product_detail.get(key) or {}).get('product_barcode') or line.product_id.barcode,
                ...
                for promotion in line.promotion_usage_ids:
                    if promotion.program_id.promotion_type == 'code' and promotion.code_id:
                        promotion_code.add(promotion.code_id.name) if promotion.code_id.name else False
        ...
        for voucher in order.pos_voucher_line_ids:
            ...
            voucher_amount += voucher.price_used
        result.update({
            ...
            'total_point': order.total_point or 0,
        })
        return []
