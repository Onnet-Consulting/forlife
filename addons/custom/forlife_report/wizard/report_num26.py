...
class ReportNum26(models.TransientModel):
    ...
    def _get_query(self):
        ...
        query = f"""
            SELECT
                ...
                st.total_package AS so_kien,
                st.total_weight AS trong_luong,
    ...
    def generate_xlsx_report(self, workbook, allowed_company):
        ...
        for value in data.get('data'):
            ...
            sheet.write(row, 7, value.get('so_kien'), formats.get('normal_format'))
            sheet.write(row, 8, value.get('trong_luong'), formats.get('normal_format'))
            sheet.write(row, 9, value.get('num'), formats.get('int_number_format'))
