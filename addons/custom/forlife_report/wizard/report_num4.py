...
class ReportNum4(models.TransientModel):
    ...
    def view_report(self):
        self.ensure_one()
        action = self.env.ref('forlife_report.report_num_4_client_action').read()[0]
        return action
...
order by num
    ...
    def generate_xlsx_report(self, workbook):
        ...
        for value in data['data']:
            ...
            sheet.write(row, 3, value['product_group'], formats.get('normal_format'))
            sheet.write(row, 4, value['color'], formats.get('normal_format'))
            sheet.write(row, 5, value['size'], formats.get('normal_format'))
            sheet.write(row, 6, value['collection'], formats.get('normal_format'))
            sheet.write(row, 7, value['texture'], formats.get('normal_format'))
            sheet.write(row, 8, value['product_line'], formats.get('normal_format'))
            sheet.write(row, 9, value['total_qty'], formats.get('float_number_format'))
            row += 1
