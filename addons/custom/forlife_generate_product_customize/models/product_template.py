...
class ProductTemplate(models.Model):
    ...
    def create(self, vals_list):
        if 'brand_id' in vals_list and 'attribute_line_ids' in vals_list:
            if vals_list['brand_id'] and vals_list['attribute_line_ids']:
                ...
                rule = False
                # attrs_dict = self.convert_attribute_line_to_dict(vals_list)
                for rec in vals_list['attribute_line_ids']:
                    if rec[2]['attribute_id'] == attribute_id.id:
                        ...
                        if rule:
                            break
                if not rule:
                    raise ValidationError('Cần phải cấu hình ít nhất một quy tắc sinh mã tự động cho sản phẩm này')
                required_field_ids = rule.required_field_ids.ids
    ...
    def convert_attribute_line_to_dict(self, vals_list):
        list_attribute_id = []
        list_value_id = []
        rslt={}
        for rec in vals_list['attribute_line_ids']:
