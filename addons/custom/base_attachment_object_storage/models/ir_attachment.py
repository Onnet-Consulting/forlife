...
from odoo.addons.base.models.ir_attachment import IrAttachment as IrAttachmentInherit
...
def write(self, vals):
    ...
    return super(IrAttachmentInherit, self).write(vals)

IrAttachmentInherit.write = write
...
class IrAttachment(models.Model):
            ...
            if storage in self._get_stores():
                if self._store_in_db_instead_of_object_storage():
                    ...
                    vals = {
                        ...
                        'index_content': False,
                        'store_fname': self._file_write(bin_data, checksum),
                        'db_datas': False,
                    }
    ...
    def _move_attachment_to_store(self):
        ...
        if fname:
            ...
            checksum = self._compute_checksum(self.datas)
            self.write({'mimetype': self.mimetype,
                        'store_fname': self._file_write(self.raw, checksum),
                        'db_datas': False
                        })
            _logger.info('store_fname: %s ', self.store_fname)
