...
class StockPickingTransferBkav(models.Model):
    ...
    def bravo_get_picking_transfer_bkav_value(self, employee_code):
        count = 1
        values = []
        for stock_move in self.move_ids:
            account_move = stock_move.account_move_ids and stock_move.account_move_ids[0]
            values.append(self.bravo_get_picking_transfer_bkav_by_account_move_value(stock_move, account_move, count, employee_code))
            count += 1
        return values
