...
class PickingOrderReturn(models.Model):
    _inherit = 'stock.picking'

    def bravo_get_picking_order_return_values(self):
        res = []
        columns = self.bravo_get_picking_order_return_columns()
        ...
        for record in self:
            ...
            for stock_move in record.move_ids:
                res.extend(record.bravo_get_picking_order_return_value(stock_move, employee.get('code'), idx))
                idx += 1
        return columns, res

    @api.model
    def bravo_get_picking_order_return_columns(self):
        return [
            "CompanyCode", "Stt", "DocCode", "DocNo", "DocDate", "CurrencyCode", "ExchangeRate", "CustomerCode", "RowId",
            "CustomerName", "Address", "TaxRegNo", "Description", "EmployeeCode", "DeptCode", "IsTransfer", "BuiltinOrder",
            "ItemCode", "ItemName", "UnitCode", "CreditAccount", "DebitAccount", "Quantity9", "ConvertRate9",
            "Quantity", "OriginalUnitCost", "UnitCost", "OriginalAmount", "Amount", "WarehouseCode", "JobCode", "RowId_Sales",
        ]

    def bravo_get_picking_order_return_value(self, stock_move, employee_code, idx):
        ...
        if self.sale_id:
            ...
            dept_code = self.pos_order_id.session_id.config_id.store_id.analytic_account_id.code
        else:
        ...
        return [{
            ...
            "Description": re.sub('<.*?>', '', self.note or '') or 'Xuất kho hàng bán trả lại',
            "EmployeeCode": employee_code or None,
            "DeptCode": stock_move.account_analytic_id.code or dept_code or None,
            ...
            "CreditAccount": credit_line.account_id.code or stock_move.product_id.categ_id.with_company(self.company_id).property_stock_account_output_categ_id.code or None,
            "DebitAccount": debit_line.account_id.code or stock_move.product_id.categ_id.with_company(self.company_id).property_stock_valuation_account_id.code or None,
            ...
            "WarehouseCode": self.location_dest_id.warehouse_id.code or None,
            "JobCode": stock_move.occasion_code_id.code or None,
            "RowId": stock_move.id or None,
            "RowId_Sales": stock_move.work_production.code or None,
        }]
