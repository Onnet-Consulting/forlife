...
//        this.loadPromotionPriceListItemBackground();
    ...
    _loadPromotionData(promotionPrograms) {
        this.promotion_program_by_id = this.promotion_program_by_id || {};
        this.reward_line_by_id = this.reward_line_by_id || {};
        this.pro_pricelist_item_by_id = this.pro_pricelist_item_by_id || {};
        var self = this;
        for (const line of this.surprisingRewardProducts) {
            line.to_check_product_ids = new Set(line.to_check_product_ids);
        };
        for (const program of promotionPrograms) {
...
//            this.promotionPricelistItems = JSON.parse(atob(json_pricelist_item_ids_str)) || [];
    ...
    get_program_by_id(str_id) {
        ...
        } else if (str_id.includes('c')) {
            let [pId, code_id] = str_id.split('c');
            let programObject = _.clone(this.promotion_program_by_id[pId]);
            programObject.code_id = code_id;
    ...
    get_str_id(program) {
        if (program.reward_type == 'code_amount' && program.code_id) {
            return `${program.id}c${program.code_id}`;
        } else {
            return program.str_id;
        };
    }
    ...
    getPromotionCode(program) {
        ...
        if (program.reward_type == 'code_amount' && program.code_id) {
            return this.get_order().activatedInputCodes.find(c=>c.id == program.code_id);
        } else {
...
const PosPromotionOrder = (Order) => class PosPromotionOrder extends Order {
    ...
    _programIsApplicableAutomatically(program, incl_finished=false) {
        if (incl_finished && program.state == 'finished' && this.finished_programs.includes(program.program_id)) {
            return true;
        };
    ...
    add_product(product, options) {
        let priceItem;
        let matched_finished_program = false;
        if (options.related_refund_line_cid) {
            // Tìm bảng giá SP đổi
            priceItem = this._getPricelistItem(product, false, true);
            if (priceItem) {
                matched_finished_program = true;
            };
        };
        if (!priceItem) {
        ...
        if (priceItem) {
            ...
            options['matched_finished_program'] = matched_finished_program;
...
//            this.remove_orderline(line);
        ...
        this._updateActivatedPromotionPrograms();
        if (has_cart_program || orderlines.length) {
    ...
    async _updateActivatedPromotionPrograms(line_no_incl_order) {
        ...
        for (let proID of Object.keys(validPromotionPrograms)) {
            if (proID.includes('p') && this.pos.get_program_by_id(proID).promotion_type === 'pricelist') {
                ...
                let codeProgram = this.pos.get_program_by_id(proID);
                if (codeProgram.reward_type == 'code_amount') {
                    let order_used_codes = this._used_codes();
                    let codes = this.activatedInputCodes.filter(code => code.program_id == codeProgram.id && !order_used_codes.includes(code.id));
                    for (let code of codes) {
                        let str_id = `${proID}c${code.id}`;
                        this.activatedCodePrograms.add(str_id);
                    };
                } else {
    ...
    getActivatedPrograms() {
        let result = [];
        let order_used_codes = this._used_codes();
        result.push(...Array.from(this.activatedComboPrograms).map(proID => this.pos.get_program_by_id(proID)));
        // Đối với CT Code giảm tiền, kiểm tra và chỉ apply CT code chưa sửa dụng
        Array.from(this.activatedCodePrograms).forEach((proID) => {
            let program = this.pos.get_program_by_id(proID);
            if (program && program.reward_type == 'code_amount' && program.code_id) {
                if (!order_used_codes.includes(program.code_id)) {
                    result.push(program);
                };
            } else {
                result.push(program);
...
//        result.push(...Array.from(this.activatedPricelistItem).map(proID => this.pos.pro_pricelist_item_by_id[proID]));
        if (this.validOnOrderPricelistItem) {
            ...
            let PricelistItemsAssigned = this.get_orderlines_to_check().reduce((tmp, l)=> {
                if (l.pricelist_item) {
                    tmp.push(l.pricelist_item.str_id);
                };
                return tmp;
            }, []);
    ...
    _get_clone_order_lines(remainingLines) {
        ...
        remainingLines.forEach((l) => {
            let pricelist_item = l.pricelist_item;
            if (pricelist_item) {
                pricelist_item['program'] = null;
                pricelist_item['pricelistItems'] = null;
            };
            lines.push({
                product: {
                    id: l.product.id,
                ...
                related_refund_line_cid: l.related_refund_line_cid,
                matched_finished_program: l.matched_finished_program
            });
        })
        return lines;
    }
...
//        this.validOnOrderPricelistItem = [];
    ...
    _getPricelistItem(product, check_with_code=false, incl_finished=false) {
        let programs = this.pos.promotionPrograms.filter(p =>
            p.promotion_type == 'pricelist'
            && (check_with_code ? p.with_code : !p.with_code));
        let pricelistItem;
...
//                if (program.promotion_type == 'code' && program.discount_based_on == 'unit_price' && usage.disc_amount) {return false};
    ...
    prepare_to_discount_line_val(line, quantity, price_unit, is_not_discount) {
        return {
            ...
            related_refund_line_cid: line.related_refund_line_cid,
            matched_finished_program: line.matched_finished_program,
        }
    }
    ...
    _checkNumberOfCode(codeProgram, order_lines, to_discount_line_vals , count, max_count = false, to_apply_lines = {}) {
        ...
        if (codeProgram.reward_type == "code_amount" && ((codeProgram.discount_apply_on == "order" && check_q >= codeProgram.min_quantity) || !valid_product_ids.size)) {
        ...
        for (const to_discount_line_val of to_discount_line_vals) {
            var key = to_discount_line_val.product.id + '-' + to_discount_line_val.promotion_usage_ids.reduce((p, {str_id}) => p + '-' + str_id, '') + '-' + to_discount_line_val.is_not_discount + '-' + to_discount_line_val.price;
    ...
    */
    ...
    _used_codes() {
        ...
        for (let line of this.orderlines) {
            let line_codes = this._get_code_ids_in_usages(line);
            if (line_codes.length) {
...
//    }
    ...
    computeBestCombineOfProgram(programsList){
        let programs = programsList.map(p => this.pos.get_str_id(p));
    ...
    _computeBestCombineOfProgram(programs_combines) {
        ...
        for (let combination of programs_combines) {
            ...
            let usage_programs = discount_lines.reduce((acc, line) => {
                    _get_program_ids_in_usages(line).forEach(el => acc.add(el)); return acc
                    }, new Set()
                );
            if (discounted_total > max) {
                result = [combination.filter(p => usage_programs.has(this.pos.get_str_id(p))).map(p => this.pos.get_str_id(p))];
    ...
    verifyProgramOnOrder(toVerifyPromotionPrograms, line_no_incl_order) {
        ...
        for (const program of toVerifyPromotionPrograms) {
            if ((this._programIsApplicableAutomatically(program) && program.promotion_type != 'cart')
                || (this.is_change_product && this.finished_programs.includes(program.program_id))) {
        ...
        for (const program of comboProgramToCheck) {
            ...
            if (program.promotion_type == 'combo') {
                ...
                if (NumberOfCombo >= 1) {
                    programIsVerified[this.pos.get_str_id(program)] = NumberOfCombo;
                };
            }
    ...
    _apply_cart_program_to_orderline(program, to_discount_lines) {
        ...
        function getDiscountedAmountAPart() {
            return to_discount_lines.reduce((tmp_line, line) => {
        ...
        if (program.reward_type == 'cart_get_voucher') {
            ...
            for (let line of to_discount_lines) {
                ...
                if (to_discount_lines.indexOf(line) == to_discount_lines.length-1) {
                    ...
                    let newPrice = round_decimals(originalPrice - remaining_amount / line.quantity, this.pos.currency.decimal_places);
                    let discAmount = originalPrice - newPrice;
                    line.price = newPrice;
                    line.promotion_usage_ids.push(new PromotionUsageLine(
                        program.id, code, null, originalPrice, newPrice, discAmount, program.str_id, program.promotion_type, program.discount_based_on));
                    line.is_cart_discounted = true;
                    if (discAmount * line.quantity + discounted != disc_total_amount) {
                        diff_amount_new_line = this.get_diff_amount_new_line(program, line, remaining_amount, discAmount, originalPrice);
                    };
                } else {
            ...
            for (let line of to_discount_lines) {
                let pricelistItem = program.pricelistItems.find(pl => pl.product_id == line.product.id);
                let originalPrice = line.price;
                let newPrice = pricelistItem.fixed_price;
                let discAmount = originalPrice - newPrice;
                line.price = newPrice;
                line.promotion_usage_ids.push(new PromotionUsageLine(
                program.id, code, null, originalPrice, newPrice, discAmount, program.str_id, program.promotion_type, program.discount_based_on));
        ...
        if (diff_amount_new_line) {
            to_discount_lines.push(diff_amount_new_line);
        }
...
//                  required_min_quantity       = floor * program.min_quantity;
            ...
            if (program.progressive_reward_compute) {
                ...
                floor = realFloor > 0 && realFloor < floor ? realFloor : floor;
            };
    ...
    get_diff_amount_new_line(CodeProgram, LineList, disc_total_amount, discAmountInLine, originalPrice) {
        ...
        let new_usage = diff_amount_new_line.promotion_usage_ids.find(u => u.str_id == this.pos.get_str_id(CodeProgram));
    ...
    applyACodeProgramToLineVales(CodeProgram, LineList, number_of_product, remaining_amount) {
        let str_id = this.pos.get_str_id(CodeProgram);
        let code = null;
        let activatedCodeObj = this.activatedInputCodes.find(c =>
            c.program_id === CodeProgram.id
            && (CodeProgram.code_id && CodeProgram.reward_type=='code_amount' ? c.id == CodeProgram.code_id : true));
        if (activatedCodeObj) {code = activatedCodeObj.id};
        let diff_amount_new_line;
...
//            }
            ...
            if (disc_total_amount > 0 || LineList.is_not_discount) {
                ...
                LineList.promotion_usage_ids.push(new PromotionUsageLine(CodeProgram.id, code, null,originalPrice, newPrice, discAmountInLine, str_id, CodeProgram.promotion_type, CodeProgram.discount_based_on));
            ...
            if (LineList.is_reward_line) {
                ...
                let newUsage = new PromotionUsageLine(CodeProgram.id, code, null, originalPrice, 0.0, discAmountInLine, str_id, CodeProgram.promotion_type, CodeProgram.discount_based_on)
                LineList.price = 0.0;
                LineList.promotion_usage_ids.push(newUsage);
            } else {
                LineList.promotion_usage_ids.push(new PromotionUsageLine(CodeProgram.id, code, null, null, null, 0, str_id, CodeProgram.promotion_type, CodeProgram.discount_based_on));
            }
...
//                    var to_apply_lines_other = Object.values(to_apply_lines);
                ...
                combo_count[this.pos.get_str_id(program)] = numberOfCombo;
...
//                                to_discount_line_val.price = (to_discount_line_val.quantity - numberOfReward) / to_discount_line_val.quantity * to_discount_line_val.price;
                    ...
                    for (let i = 0; i < to_discount_line_vals.length; i++) {
                        ...
                        if (to_apply_lines.hasOwnProperty(this.pos.get_str_id(program)) && combo_count.hasOwnProperty(this.pos.get_str_id(program))) {
                            to_apply_lines[this.pos.get_str_id(program)].push(...result);
                        } else {
                            to_apply_lines[this.pos.get_str_id(program)] = result;
                        };
                    };
//                }
            }
    ...
    autoApplyPriceListProgram(new_ol) {
        ...
        if (new_ol && new_ol.quantity > 0 && !new_ol.is_applied_promotion() && new_ol.pricelist_item) {
            if (this._programIsApplicableAutomatically(new_ol.pricelist_item, new_ol.matched_finished_program) && !is_with_code(new_ol.pricelist_item)) {
        ...
        if (!new_ol) {
            ...
            for (let line of to_check_orderlines) {
                if (this._programIsApplicableAutomatically(line.pricelist_item)
                    && !is_with_code(line.pricelist_item)) {
...
//        vals['lst_price'] = vals['price'];
    ...
    _getNewLineValuesAfterDiscount(arg){
        ...
        return {
            ...
            related_refund_line_cid: arg.related_refund_line_cid,
            matched_finished_program: arg.matched_finished_program
        }
    }
