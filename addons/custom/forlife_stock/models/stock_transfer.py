...
class StockTransfer(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            ...
            vals['name'] = (self.env['ir.sequence'].next_by_code('stock.transfer.sequence') or 'PXB') + (warehouse if warehouse else '') + str(
                datetime.now().year)
...
class StockTransferLine(models.Model):
    ...
    def validate_product_quantity(self, location=False, is_diff_transfer=False):
        ...
        quantity_prodution = self.env['quantity.production.order'].search([('product_id', '=', product.id), ('location_id', '=', self.stock_transfer_id.location_id.id), ('production_id', '=', self.work_from.id)])
        if quantity_prodution:
            if self.work_from and self.qty_out > quantity_prodution.quantity:
                raise ValidationError('Đang test dở!')
        else:
            raise ValidationError('Sản phẩm không có trong lệnh sản xuất!')
