...
class ImportTaxInfoWizard(models.TransientModel):
    ...
    def generate_xlsx_report(self, workbook, allowed_company, **kwargs):
        ...
        sheet.set_row(0, 30)
        TITLES = [
            'Database ID', 'Sản phẩm', 'Barcode', 'Mô tả', 'Thành tiền VND', 'Số lượng', '% Thuế nhập khẩu', 'Thuế nhập khẩu',
        ...
        for line in self.po_id.exchange_rate_line_ids:
            ...
            sheet.write(row, 2, line.product_id.barcode or '', formats.get('normal_format'))
            sheet.write(row, 3, line.description or '', formats.get('normal_format'))
            sheet.write(row, 4, line.total_vnd_exchange or 0, formats.get('int_number_format'))
            sheet.write(row, 5, line.product_qty or 0, formats.get('int_number_format'))
            sheet.write(row, 6, line.import_tax or 0, formats.get('float_number_format'))
            sheet.write(row, 7, line.tax_amount or 0, formats.get('int_number_format'))
            sheet.write(row, 8, line.special_consumption_tax or 0, formats.get('float_number_format'))
            sheet.write(row, 9, line.special_consumption_tax_amount or 0, formats.get('int_number_format'))
            sheet.write(row, 10, line.vat_tax or 0, formats.get('float_number_format'))
            sheet.write(row, 11, line.vat_tax_amount or 0, formats.get('int_number_format'))
            sheet.write(row, 12, line.total_tax_amount or 0, formats.get('int_number_format'))
            row += 1

    def get_filename(self):
        return f"Đơn mua hàng {self.po_id.name or ''}"
    ...
    def action_import(self):
        ...
        for index, data in enumerate(data_import, start=2):
            if not data[0] or not old_tax_data.get(data[0]):
                ...
                import_tax = float(data[6])
                special_consumption_tax = float(data[8])
                vat_tax = float(data[10])
