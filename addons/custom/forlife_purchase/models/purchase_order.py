...
class PurchaseOrder(models.Model):
    ...
    def compute_count_invoice_inter_normal_fix(self):
        for rec in self:
            rec.count_invoice_inter_normal_fix = self.env['account.move'].search_count(
                [('purchase_order_product_id', 'in', rec.id), ('move_type', '=', 'in_invoice'), ('select_type_inv', '=', 'normal')])
    ...
    def action_create_invoice(self):
        ...
        if len(self) > 1 and self[0].type_po_cost in ('cost', 'tax'):
            ...
            if self.select_type_inv == 'service' and self.purchase_type in ('service', 'asset'):
                ...
                for order in self:
                    ...
                    if order.select_type_inv == 'labor':
                        picking_labor_in = self.env['stock.picking'].search([('origin', '=', order.name),
                                                                       ('state', '=', 'done'),
                                                                       ('labor_check', '=', True),
                                                                       ('x_is_check_return', '=', False),
                                                                       ('picking_type_id.code', '=', 'incoming')
                                                                       ])
                        picking_labor_in_return = self.env['stock.picking'].search([('origin', '=', order.name),
                                                                              ('state', '=', 'done'),
                                                                              ('labor_check', '=', True),
                                                                              ('picking_type_id.code', '=', 'incoming'),
                        ...
                        if order.order_line_production_order:
                            for line in order.order_line_production_order:
                                ...
                                if not order.is_return:
                                    ...
                                    wave = picking_labor_in.move_line_ids_without_package.filtered(lambda w: str(w.po_id) == str(line.id)
                                                                                                    and w.product_id.id == line.product_id.id
                                                                                                    and w.picking_id.x_is_check_return == False)
                                        ...
                                        for wave_item in wave:
                                            ...
                                            if purchase_return:
                                                for x_return in purchase_return:
                                                    if wave_item.picking_id.name == x_return.picking_id.relation_return:
                                                        data_line = {
                                                            ...
                                                            'quantity': 1,
                                                            'sequence': sequence,
                                                            'price_unit': material_line.price_unit * ((wave_item.qty_done - x_return.qty_done) / line.purchase_quantity),
                                                        ...
                                                        if pending_section:
                                                            ...
                                                            sequence += 1
                                                            pending_section = None
                                                        # x_return.picking_id.labor_check = True
                                                        # wave.picking_id.labor_check = True
                                                        line_vals = line._prepare_account_move_line()
                                                        line_vals.update(data_line)
                                                        invoice_vals['invoice_line_ids'].append((0, 0, line_vals))
                                                        sequence += 1
                                                ...
                                                data_line = {
                                                    ...
                                                    'quantity': 1,
                                                    'sequence': sequence,
                                                    'price_unit': material_line.price_unit * (wave_item.qty_done / line.purchase_quantity),
                                                    'occasion_code_id': wave_item.occasion_code_id.id,
                                                    'work_order': wave_item.production_id.id,
                                                    'account_analytic_id': wave_item.account_analytic_id.id,
                                                }
                        ...
                        picking_expense_in = self.env['stock.picking'].search([('origin', '=', order.name),
                                                                             ('state', '=', 'done'),
                                                                             ('expense_check', '=', True),
                                                                             ('x_is_check_return', '=', False),
                                                                             ('picking_type_id.code', '=', 'incoming')
                                                                             ])
                        picking_expense_in_return = self.env['stock.picking'].search([('origin', '=', order.name),
                                                                                    ('state', '=', 'done'),
                                                                                    ('expense_check', '=', True),
                                ...
                                for line in order.order_line:
                                    ...
                                    for wave_item in wave:
                                        ...
                                        if purchase_return:
                                            for x_return in purchase_return:
                                                if wave_item.picking_id.name == x_return.picking_id.relation_return:
                                                    cp = ((line.total_vnd_amount / sum(order.order_line.mapped('total_vnd_amount'))) * ((wave_item.qty_done - x_return.qty_done) / line.purchase_quantity)) * nine.vnd_amount
                                                    data_line = {
                                                        'ware_name': wave_item.picking_id.name,
                                                        'po_id': line.id,
                                                    ...
                                                    if pending_section:
                                                        ...
                                                        sequence += 1
                                                        pending_section = None
                                                    # x_return.picking_id.expense_check = True
                                            ...
                                            data_line = {
                                                ...
                                                'sequence': sequence,
                                                'quantity': 1,
                                                'price_unit': ((line.total_vnd_amount / sum(order.order_line.mapped('total_vnd_amount'))) * (wave_item.qty_done / line.purchase_quantity)) * nine.vnd_amount,
                                                'occasion_code_id': wave_item.occasion_code_id.id,
                                                'work_order': wave_item.production_id.id,
                                                'account_analytic_id': wave_item.account_analytic_id.id,
                                            }
                        ...
                        picking_in_return = self.env['stock.picking'].search([('origin', '=', order.name),
                                                                              ('state', '=', 'done'),
                                                                              ('ware_check', '=', False),
                                                                              ('picking_type_id.code', '=', 'incoming'),
                                                                              ('x_is_check_return', '=', True)
                                                                              ])
                        picking_in_true = self.env['stock.picking'].search([('origin', '=', order.name),
                                                                            ('state', '=', 'done'),
                                                                            ('ware_check', '=', True),
                                                                            ('x_is_check_return', '=', False),
                                                                            ('picking_type_id.code', '=', 'incoming')
                        ...
                        invoice_relationship = self.env['account.move'].search([('reference', '=', order.name),
                                                                              ...
                                                                              ('receiving_warehouse_id', 'in', picking_in_true.ids)
                                                                              ])
                        for line in order.order_line:
                            if not order.is_return:
                            ...
                            if order.select_type_inv == 'normal':
                                if invoice_relationship and (sum(invoice_relationship.invoice_line_ids.mapped('quantity')) == sum(picking_in_true.move_line_ids_without_package.mapped('qty_done'))):
                                    raise UserError(_('Hóa đơn đã được khống chế số lượng theo phiếu nhập kho!'))
                                else:
                                    if picking_in:
                                        for wave_item in wave:
                                    ...
                                    # else:
                                    #     raise UserError(
                                    #         _('Đơn mua đã có hóa đơn liên quan tương ứng với phiếu nhập kho!'))
                # 2) group by (company_id, partner_id, currency_id) for batch creation
                new_invoice_vals_list = []
                picking_stock = self.env['stock.picking'].search([('origin', '=', self.name),
                                                                       ('state', '=', 'done'),
                                                                       ('x_is_check_return', '=', False),
                                                                       ('picking_type_id.code', '=', 'incoming')
                                                                       ])
                if not order.is_return:
                    picking_stock = picking_stock.filtered(lambda r: r.origin == order.name
                                                                   and r.state == 'done'
                    ...
                    picking_stock = picking_stock.filtered(lambda r: r.origin == order.name
                                                                     ...
                                                                     and r.ware_check == True
                                                                     and r.x_is_check_return == False)                                                       
                picking_incoming_normal_check = picking_stock.filtered(lambda r: r.origin == order.name
                                                                                  and r.state == 'done'
                                                                                  and r.picking_type_id.code == 'incoming'
                                                                                  and r.ware_check == True
                                                                                  and r.x_is_check_return == False)
                picking_incoming_expense_check = picking_stock.filtered(lambda r: r.origin == order.name
                                                                    and r.state == 'done'
                                                                    and r.picking_type_id.code == 'incoming'
                                                                    and r.expense_check == True
                                                                    and r.ware_check == False
                                                                    and r.x_is_check_return == False)
                picking_incoming_labor_check = picking_stock.filtered(lambda r: r.origin == order.name
                                                                    and r.state == 'done'
                                                                    and r.picking_type_id.code == 'incoming'
                                                                    and r.labor_check == True
                                                                    and r.ware_check == False
                                                                    and r.x_is_check_return == False)
                list_picking_incoming_normal = []
                list_picking_incoming_expense = []
                list_picking_incoming_labor = []
                for normal in picking_incoming_normal_check:
                    list_picking_incoming_normal.append(normal.id)
                for expense in picking_incoming_expense_check:
                    list_picking_incoming_expense.append(expense.id)
                for labor in picking_incoming_labor_check:
                    list_picking_incoming_labor.append(labor.id)
                for grouping_keys, invoices in groupby(invoice_vals_list, key=lambda x: (
                        x.get('company_id'), x.get('partner_id'), x.get('currency_id'))):
                    origins = set()
                    payment_refs = set()
    ...
    def create_multi_invoice_vendor(self):
                    ...
                    for line in order.order_line_production_order:
                        ...
                        for material_line in material:
                            if material_line.product_id.product_tmpl_id.x_type_cost_product == 'labor_costs' and picking_labor_in:
                                for wave_item in wave:
                                    # purchase_return = picking_labor_in_return.move_line_ids_without_package.filtered(
                                    #     lambda r: str(r.po_id) == str(wave.po_id)
                                    #               and r.product_id.id == wave.product_id.id
                                    #               and r.picking_id.relation_return == wave.picking_id.name
                                    #               and r.picking_id.x_is_check_return == True)
                                    # if purchase_return:
                                    #     for x_return in purchase_return:
                                    #         if wave.picking_id.name == x_return.picking_id.relation_return:
                                    #             data_line = {
                                    #                 'ware_name': wave_item.picking_id.name,
                                    #                 'po_id': line.id,
                                    #                 'product_id': material_line.product_id.id,
                                ...
                                raise UserError(_('Đơn mua có mã phiếu là %s chưa có chi phí nhân công!') % order.name)
                            sequence += 1
                            key = order.purchase_type, order.partner_id.id, order.company_id.id
                            invoice_vals = order._prepare_invoice()
                            invoice_vals.update({'purchase_type': order.purchase_type,
        ...
        for data in vals_all_invoice:
            ...
            for line in move:
                ...
                picking_expense_in = self.env['stock.picking'].search([('origin', 'in', line.purchase_order_product_id.mapped('name')),
                                                                       ('state', '=', 'done'),
                                                                       ('expense_check', '=', True),
                                                                       ('x_is_check_return', '=', False),
                                                                       ('picking_type_id.code', '=', 'incoming')
                                                                       ])
                picking_labor_in = self.env['stock.picking'].search([('origin', 'in', line.purchase_order_product_id.mapped('name')),
                                                                       ('state', '=', 'done'),
                                                                       ('labor_check', '=', True),
                                                                       ('x_is_check_return', '=', False),
                                                                       ('picking_type_id.code', '=', 'incoming')
                                                                       ])
                picking_normal_in = self.env['stock.picking'].search(
                    [('origin', 'in', line.purchase_order_product_id.mapped('name')),
                     ('state', '=', 'done'),
                     ('ware_check', '=', True),
                     ('x_is_check_return', '=', False),
                     ('picking_type_id.code', '=', 'incoming')
