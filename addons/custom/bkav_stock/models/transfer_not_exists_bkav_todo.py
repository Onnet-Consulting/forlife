...
class TransferNotExistsBkav(models.Model):
    ...
    def general_transfer_not_exists_bkav(self):
        ...
        """
        self._cr.execute(query, (date_now))
        transfer_ids = self.env['transfer.not.exists.bkav'].search([('state','=','new')],order='id asc')
        for transfer_id in transfer_ids:
            location_id = transfer_id.location_id.id
            location_dest_id = transfer_id.location_dest_id.id
            query_transfer = """
                SELECT s.id
                FROM stock_transfer s
                WHERE s.exists_bkav = 'f' 
                ...
                AND s.state in ('out_approve','in_approve','done');
            """
            self._cr.execute(query_transfer, (location_id,location_dest_id,date_now))
            result = self._cr.fetchall()
            stock_transfer_ids = self.env['stock.transfer'].sudo().browse([idd[0] for idd in result])
            list_ids = []
            list_line_product, stock_transfer_ids, check_loop = self._split_transfer_line(stock_transfer_ids)
            while check_loop:
                list_line_product, stock_transfer_ids, check_loop = self._split_transfer_line(stock_transfer_ids)
            for item in list_line_product:
                product_id = item.get('product_id')
                uom_id = item.get('uom_id')
                quantity = item.get('quantity')
                #Todo: Tạo line bảng tỏng hop
