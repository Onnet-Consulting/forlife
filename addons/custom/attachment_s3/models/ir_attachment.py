...
class IrAttachment(models.Model):
    ...
    def _get_s3_connection_params(self, bucket_name=None):
        aws_config = self.env['aws.config'].sudo().search([], limit=1)
        host = aws_config.aws_host if aws_config else ''

        # Ensure host is prefixed with a scheme (use https as default)
        if host and not urlsplit(host).scheme:
            host = 'https://%s' % host

        region_name = aws_config.aws_region if aws_config else ''
        access_key = aws_config.aws_access_key_id if aws_config else ''
        secret_key = aws_config.aws_secret_access_key if aws_config else ''
        aws_use_irsa = os.environ.get('AWS_USE_IRSA')
        bucket_name = bucket_name or (aws_config.aws_bucket if aws_config else '')
